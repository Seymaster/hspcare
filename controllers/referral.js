...
exports.getAllReferral = (req,res,next)=>{
    Referral.find()
    .then(data =>{
        if(data === null){
            res.status(200).send({
    ...
    .catch(err =>{
        res.status(500).send({
            status: 500,
            message: "Error loading Referral",
            err: err
        })
    });
};
...
exports.postReferral = (req,res,next) =>{
    const {name,occupation,company,address,phoneNumber,email} = req.body;
    const newReferral = Referral({name,occupation,company,address,phoneNumber,email});
    newReferral.save((err,data)=>{
        if(err){
            if (err.code == 11000) {
                let error = err['errmsg'].split(':')[2].split(' ')[1].split('_')[0];
                res.status(500).send({
                    message: `${error} Referral saved already`,
                    status: 11000,
                    error: err
                });
                return false;
            ...
            res.status(500).send({
                status:500,
                message:"Could not save Referral"
            });
        }
        else{
            // console.log(data)
